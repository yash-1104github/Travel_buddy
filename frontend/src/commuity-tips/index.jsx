...
import { getCommunityTrips } from "@/service/tripAPI";
...
export default function CommnityTrips() {
  ...
  const GetTrips = async () => {
    try {
      const response = await getCommunityTrips();
      console.log("Community Trips:", response.data);
      setCommunityTrips(response.data);
    } catch (error) {
      console.error("Failed to fetch community trips:", error);
      toast("Failed to load community trips. Please try again.");
    }
  };
